...
public class BaikalAppManager { 
    ...
    BaikalAppProfile mTopAppProfile = new BaikalAppProfile("system",1000);
    ...
    public void onTopAppChangedLocked(String packageName, int uid) {
        ...
        if( uid != mTopAppUid || !packageName.equals(mTopAppPackageName) ) {
            ...
            BaikalAppProfile profile = mService.getBaikalAppProfileNotNullInternal(packageName,uid);
            
            if( BaikalConstants.BAIKAL_DEBUG_APP_PROFILE ) Slog.i(TAG,"setTopApp:" + BaikalAppProfileHelper.dumpProfile(profile));
    ...
    private void updateThermalLocked() {
        if( mBaikalThermalManager != null && mTopAppProfile != null ) {
            mBaikalThermalManager.updateThermalProfile(String.valueOf(mTopAppProfile.mThermalProfile));
        }
    }
    ...
    public boolean isApplicationBackgroundRestricted(int uid, String packageName) {
        ...
        if( mService.isUidActive(uid) ) return false;
