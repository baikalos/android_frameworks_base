...
public class BaikalThermalManager {
    ...
    private String updateKernelFsProfile(String profile) {
        if( BaikalConstants.BAIKAL_DEBUG_APP_PROFILE ) Slog.i(TAG,"updateKernelFsProfile: profile=" + profile + ", mCurrentThermalProfile=" + mCurrentThermalProfile);
        try {
            if( mKernelFSPath != null && !"".equals(mKernelFSPath) &&
                profile != null && !"".equals(profile) ) {
                if( !profile.equals(mCurrentThermalProfile) ) {
                    if( BaikalConstants.BAIKAL_DEBUG_APP_PROFILE ) Slog.i(TAG,"updateKernelFsProfile: write profile=" + profile);
                    FileUtils.stringToFile(mKernelFSPath, profile);
                    mCurrentThermalProfile = profile;
                }
                return profile;
            } else {
                Slog.e(TAG, "Failed to update thermal profile " + profile + " to " + mKernelFSPath);
            }
        } catch (Exception e) {
    ...
    private String updateSystemPropertyProfile(String profile) {
        if( BaikalConstants.BAIKAL_DEBUG_APP_PROFILE ) Slog.i(TAG,"updateSystemPropertyProfile: profile=" + profile + ", mCurrentThermalProfile=" + mCurrentThermalProfile);
        try {
            if( mPropertyName != null && !"".equals(mPropertyName) &&
                profile != null && !"".equals(profile) ) {
                if( !profile.equals(mCurrentThermalProfile) ) {
                    if( BaikalConstants.BAIKAL_DEBUG_APP_PROFILE ) Slog.i(TAG,"updateSystemPropertyProfile: write profile=" + profile);
                    SystemProperties.set(mPropertyName, profile);
                    mCurrentThermalProfile = profile;
                }
                return profile;
            } else {
                Slog.e(TAG, "Failed to update thermal profile " + profile + " to " + mPropertyName);
            }
        } catch (Exception e) {
