...
public class BaikalAppProfileService extends SystemService {
    ...
    private BaikalAppProfile mSystemProfile;
    private Object mSystemProfileLockObj = new Object();
    ...
    BaikalAppProfile getProfileInternal(int uid) {
        try {
            ...
            synchronized (mProfiles) {
                if( uid == 1000 ) {
                    profile = getSystemProfileInternal();
                } else {
    ...
    BaikalAppProfile getSystemProfileInternal() {
        if( mSystemProfile != null ) return mSystemProfile;
        synchronized(mSystemProfileLockObj) {
            if( mSystemProfile != null ) {
                return mSystemProfile;
            } 
            BaikalAppProfile profile = new BaikalAppProfile(1000);
            //profile.mAppInfo |= BaikalAppProfile.BAIKAL_APPINFO_IS_DEFAULT_PROFILE;
            profile.mAppInfo |= BaikalAppProfile.BAIKAL_APPINFO_IS_SYSTEM;
            profile.mAppInfo |= BaikalAppProfile.BAIKAL_APPINFO_IS_SYSTEM_WITELISTED;
            mSystemProfile = profile;
        }
        return mSystemProfile;
    }
