...
final class DefaultPermissionGrantPolicy {
    ...
    static {
        ...
        PHONE_PERMISSIONS.add(Manifest.permission.MANAGE_ONGOING_CALLS);
        PHONE_PERMISSIONS.add(Manifest.permission.USE_ICC_AUTH_WITH_DEVICE_IDENTIFIER);
    }
    ...
    static {
        ...
        STORAGE_PERMISSIONS.add(Manifest.permission.WRITE_MEDIA_STORAGE);
        STORAGE_PERMISSIONS.add(Manifest.permission.READ_MEDIA_VISUAL_USER_SELECTED);
        STORAGE_PERMISSIONS.add(Manifest.permission.MEDIA_ROUTING_CONTROL);
    }
	...
	private static final Set<String> FILEACCESS_PERMISSIONS_MES = new ArraySet<>();
    static {
        FILEACCESS_PERMISSIONS_MES.add(Manifest.permission.MANAGE_EXTERNAL_STORAGE);
    }
    ...
    static {
        ...
        //GMS_PERMISSIONS.add(Manifest.permission.GET_USAGE_STATS);
    ...
    public void grantDefaultPermissions(int userId) {
        ...
        Log.i(TAG, "Granting Default permissions to platform signature apps for user :" + userId);
    ...
    public void grantDefaultPermissionsBaikalPackage(int userId, String packageName, BaikalAppProfile profile) {
        DelayingPackageManagerCache pm = new DelayingPackageManagerCache();

        grantDefaultSystemHandlerPackagePermissionsBaikal(pm, userId, packageName, profile);
    ...
    private void grantAllRuntimePermissionsForSystemPackage(PackageManagerWrapper pm,
        ...
        for (String permission : pkg.requestedPermissions) {
            ...
            if (perm == null
                ...
                Log.i(TAG, "grantAllRuntimePermissionsForSystemPackage: Permission not found for package=" + pkg.packageName + ", perm=" + perm);
                continue;
            }
            if (perm.isRuntime()) {
                permissions.add(permission);
            } else {
                Log.i(TAG, "grantAllRuntimePermissionsForSystemPackage: Not runtime permission for package=" + pkg.packageName + ", perm=" + perm);
            }
        }
    ...
    private void grantRuntimePermissionsForSystemPackage(PackageManagerWrapper pm,
        ...
        grantAllRuntimePermissionsForSystemPackage(pm,userId,pkg,filterPermissions);
        ...
        for (String permission : filterPermissions) {
            ...
            if (perm == null
                    || (filterPermissions != null && !filterPermissions.contains(permission))) {
                Log.i(TAG, "grantRuntimePermissionsForSystemPackage: Permission not found for package=" + pkg.packageName + ", perm=" + perm);
                continue;
            }
            if (perm.isRuntime()) {
                permissions.add(permission);
            } else {
                Log.i(TAG, "grantRuntimePermissionsForSystemPackage: Not runtime permission for package=" + pkg.packageName + ", perm=" + perm);
            }
        }
        if (!permissions.isEmpty()) {
            grantRuntimePermissions(pm, pkg, permissions, true /*systemFixed*/, userId);
    ...
    private void grantGmsPermissions(PackageManagerWrapper pm, String packageName, int userId) {
        Log.i(TAG, "grantGmsPermissions: Granting default permissions to gms for user " + userId);
    ...
    private void grantDefaultSystemHandlerPackagePermissionsBaikal(PackageManagerWrapper pm, int userId, String packageName, BaikalAppProfile profile) {
        //if( profile.mFileAccess != 0 ) 
    }
    ...
    private abstract class PackageManagerWrapper {
        ...
        boolean isSystemPackage(@Nullable PackageInfo pkg) {
            ...
            return pkg.applicationInfo.isSystemApp() || pkg.applicationInfo.isUpdatedSystemApp();// || pkg.applicationInfo.isApex() || pkg.applicationInfo.isPrivileged();
            ...
            void apply() {
                if (DEBUG) {
                    Slog.i(TAG, "Granting " + mPermission + " to user " + mUser.getIdentifier()
                            ...
                            + " flags=" + (newFlags != null ? Integer.toBinaryString(newFlags) : "null"));
                }
