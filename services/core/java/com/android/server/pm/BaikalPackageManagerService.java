...
public final class BaikalPackageManagerService {
    ...
    public BaikalAppProfile pmGetAppProfile(int uid) {
        //Log.w(TAG,"snapshot: Baikal pmGetAppProfile");
        Computer snapshot = mPm.snapshotComputer();
        String [] packages = snapshot.getPackagesForUid(UserHandle.getAppId(uid));
        if( packages != null && packages.length > 0 ) {
            return new BaikalAppProfile(uid);
        }
        return null;
    }

    public String[] pmGetAppPackages(int uid) {
        //Log.w(TAG,"snapshot: Baikal pmGetAppPackages");
        Computer snapshot = mPm.snapshotComputer();
        String [] packages = snapshot.getPackagesForUid(UserHandle.getAppId(uid));
        return packages;
    }

    public boolean isSystemPackage(int uid) {
        //Log.w(TAG,"snapshot: Baikal isSystemPackage" + uid);
        Computer snapshot = mPm.snapshotComputer();
        String [] packages = snapshot.getPackagesForUid(UserHandle.getAppId(uid));
        if( packages != null && packages.length > 0 ) {
            isSystemPackage(packages[0]);
        }
        return false;
    }

    public boolean isSystemPackage(String packageName) {
        //Log.w(TAG,"snapshot: Baikal isSystemPackage " + packageName);
        Computer snapshot = mPm.snapshotComputer();
    ...
    BaikalAppProfile getBaikalUserAppPofile(String packageName) {
        return mBaikal.getUserProfile(packageName);
    }
    ...
    boolean isAllowSigOverride(int appid) {
        BaikalAppProfile profile = mBaikal.getUserProfile(appid);  //  mBaikal.getBaikalAppProfileNotNull(appid);
