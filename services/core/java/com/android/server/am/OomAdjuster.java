...
public class OomAdjuster {
        ...
        for (int i = numLru - 1; i >= 0; i--) {
            ...
            if (!app.isKilledByAm() && app.getThread() != null) {
                ...
                if( state.getCurProcState() <= ActivityManager.PROCESS_STATE_BOUND_FOREGROUND_SERVICE ) {
                    ...
                    app.mBaikalAppProfile.setLastTopTime(now);
                }
                ...
                if( !app.mBaikalAppProfile.mPinned && !(uidRec == null || uidRec.isCurAllowListed()) ) {
                    ...
                    if( (app.mBaikalAppProfile.mDebug && BaikalConstants.BAIKAL_DEBUG_OOM) || BaikalConstants.BAIKAL_DEBUG_OOM_RAW ) {
                        /*if( state.getCurProcState() > ActivityManager.PROCESS_STATE_CACHED_ACTIVITY ) {*/
                            ...
                            Slog.d(TAG_OOM_ADJ, "OOM app check packageName=" + app.mBaikalAppProfile.mPackageName + "/" + app.mBaikalAppProfile.mUid +
                                                ...
                                                ", state=" + ActivityManager.procStateToString(state.getCurProcState()) + 
                                                ", dnc=" + app.mBaikalAppProfile.mDoNotClose +
                                                ", awi=" + app.mBaikalAppProfile.mAllowWhileIdle +
                                                ...
                                                ", timeoutExtreme=" + timeoutExtreme + 
                                                ", timeoutStamina=" + timeoutStamina + 
                                                ", timeoutStartup=" + timeoutStartup );
                        /*}*/
                    }

                    if( mService.mBaikalAppProfileManager.isStamina() 
                        && !app.mBaikalAppProfile.mImportantApp
                        && !app.mBaikalAppProfile.getStamina()
                    ...
                    if( !killed 
                        ...
                        /*if( state.getCurProcState() >= ActivityManager.PROCESS_STATE_CACHED_ACTIVITY
                            && ( state.getCurAdj() >= (ProcessList.CACHED_APP_MIN_ADJ + 0))
                            ...
                            && (app.getLastActivityTime() > now 
                                ...
                                app.killLocked("baikalos - expired startup process",
                                "baikalos - expired startup process",
                                ApplicationExitInfo.REASON_OTHER,
                                ApplicationExitInfo.SUBREASON_KILL_BACKGROUND,
                                true);
                                killed = true;
                            ...
                            && app.getLastActivityTime() < oldTimeExtreme )  {
                                app.killLocked("baikalos - cached background process - extreme awake",
                                "baikalos - cached background process - extreme awake",
                                ApplicationExitInfo.REASON_OTHER,
                                ApplicationExitInfo.SUBREASON_KILL_BACKGROUND,
                                true);
                                killed = true;
                    ...
                    if( !killed 
                        ...
                        && !app.mBaikalAppProfile.mAllowWhileIdle
                        && !app.mBaikalAppProfile.mImportantApp
                        && mService.mBaikalAppProfileManager.getCurrentProfile().isHeavy() ) {
                        if( app.mBaikalAppProfile.getBackgroundMode() >= 0
                            && state.getCurProcState() >= ActivityManager.PROCESS_STATE_CACHED_ACTIVITY ) {
                ...
                if ( !app.mBaikalAppProfile.mPinned && app.isolated && psr.numberOfRunningServices() <= 0
    ...
    private boolean computeOomAdjLSP(ProcessRecord app, int cachedAdj,
        ...
        if (state.getCachedIsPreviousProcess() && state.getCachedHasActivities()) {
            if (procState >= ActivityManager.PROCESS_STATE_LAST_ACTIVITY
                    && state.getSetProcState() == ActivityManager.PROCESS_STATE_LAST_ACTIVITY
                    && (state.getLastStateTime() + /*mConstants.MAX_PREVIOUS_TIME*/ 120 * 1000) < now) {
        ...
        //if (true) Slog.i(TAG, "OOM " + app + ": initial adj=" + adj
        //        + " reason=" + state.getAdjType() + " home=" + state.getCachedIsHomeProcess());
            ...
            if (s.startRequested) {
                ...
                if (!s.mKeepWarming && state.hasShownUi() && !state.getCachedIsHomeProcess()) {
                    ...
                    if (adj > ProcessList.SERVICE_ADJ) {
                        ...
                        if (DEBUG_OOM_ADJ_REASON || logUid == appUid) {
                            reportOomAdjMessageLocked(TAG_OOM_ADJ, "Raise adj to cch-started-ui-services: " + app);
                        }
                    }
                } else {
                    if (s.mKeepWarming
                    ...
                    if (adj > ProcessList.SERVICE_ADJ) {
                        ...
                        if (DEBUG_OOM_ADJ_REASON || logUid == appUid) {
                            reportOomAdjMessageLocked(TAG_OOM_ADJ, "Raise adj to cch-started-services: " + app);
                        }
                    }
                }
            }
                ...
                for (int i = 0;
                    ...
                    if ((cr.flags & Context.BIND_WAIVE_PRIORITY) == 0) {
                        ...
                        if (adj > clientAdj) {
                            ...
                            if (state.hasShownUi() && !state.getCachedIsHomeProcess()
                                ...
                                if (adj > newAdj) {
                                    ...
                                    adjType = "service";
                                    if (DEBUG_OOM_ADJ_REASON || logUid == appUid) {
                                        reportOomAdjMessageLocked(TAG_OOM_ADJ, "Raise adj to service : " + app);
                                    }
                                }
                            }
                        }
                        ...
                        if (procState > clientProcState) {
                            ...
                            if (DEBUG_OOM_ADJ_REASON || logUid == appUid) {
                                reportOomAdjMessageLocked(TAG_OOM_ADJ, "Raise procstate to service : " + app);
                            }
                        }
        ...
        app.mBaikalAppProfile.setCurAdj(adj);

        if( app.mBaikalAppProfile.mAllowIdleNetwork || app.mBaikalAppProfile.getBackgroundMode() < 0  ) {
            capability |= PROCESS_CAPABILITY_NETWORK;
        }
    ...
    private boolean applyOomAdjLSP(ProcessRecord app, boolean doingAll, long now,
        ...
        if (state.getCurAdj() != state.getSetAdj()) {
            ProcessList.setOomAdj(app.getPid(), app.uid, state.getCurAdj(), app.mBaikalAppProfile.mPinned || app.mBaikalAppProfile.mDoNotClose);
        ...
        processGroup = mService.mBaikalAppProfileManager.updateProcSchedGroup(state.getCurProcState(), state.getCurAdj(),app.mBaikalAppProfile, processGroup, curSchedGroup);
        ...
        if (curBoundByNonBgRestrictedApp != state.isSetBoundByNonBgRestrictedApp()) {
            ...
            if (!curBoundByNonBgRestrictedApp && app.isBackgroundRestricted()) {
        ...
        if (state.isCached() && !state.shouldNotKillOnBgRestrictedAndIdle()) {
            ...
            if (!state.isSetCached() || state.isSetNoKillOnBgRestrictedAndIdle() || 
                state.getLastCanKillOnBgRestrictedAndIdleTime() == 0 ) {
    ...
    void idleUidsLocked() {
        ...
        for (int i = N - 1; i >= 0; i--) {
            ...
            if( BaikalPowerSaverPolicyConfig.getCurrentPowerSaverPolicyConfig().lessRestrictiveBackgroundPolicy ) {
                if( uidRec.getBaikalAppProfile().mBackgroundMode <= -1 ) continue;
                if( uidRec.getBaikalAppProfile().mSystemWhitelisted ) continue;
            }
            ...
            if( uidRec.isIdle() ) {
                uidRec.forEachProcess(app -> {
                    mProcessList.killAppIfBgRestrictedAndCachedIdleLocked(app, nowElapsed);
                });
            }
        }
        if (mLocalPowerManager != null) {
    ...
    private void updateAppFreezeStateLSP(ProcessRecord app, String oomAdjReason) {
        ...
        if (opt.shouldNotFreeze() || app.mBaikalAppProfile.mDisableFreezer || app.mBaikalAppProfile.getBackgroundMode() < 0) {
            if (opt.isFrozen() || opt.isPendingFreeze()) {
                mCachedAppOptimizer.unfreezeAppLSP(app, oomAdjReason);
            }
            return;
