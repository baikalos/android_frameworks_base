...
public final class BaikalContext { 
    ...
    public BaikalContext(@NonNull Context context,  
        ...
        if( UserHandle.getAppId(android.os.Process.myUid()) == android.os.Process.SYSTEM_UID ) {
            mBaikalProfile = new BaikalAppProfile("android",android.os.Process.SYSTEM_UID);
                ...
                if( mBaikalProfile == null ) {
                    if( mBaikalService == null || mBaikalAppProfileService == null ) {
                        ...
                        if( mBaikalService == null || mBaikalAppProfileService == null ) {
                            ...
                            return new BaikalAppProfile(mBasePackageName, android.os.Process.myUid());
                        }
                    }
                    try {
                        mBaikalProfile = mBaikalAppProfileService.getProfileNotNull(android.os.Process.myUid());
                    } catch(Exception e) {
                        Log.e(TAG,"Baikal getProfileNotNull exception:", e);
                    }
                    if(mBaikalProfile == null ) {
     ...
     */
    @Nullable
    public BaikalAppProfile getBaikalAppProfile(@Nullable String packageName, int uid) {
        try {
            return mBaikalService.getBaikalAppProfile(packageName,uid);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     * 
     */
    public BaikalAppProfile getBaikalAppProfileNotNull(@Nullable String packageName, int uid) {
        try {
            return mBaikalService.getBaikalAppProfileNotNull(packageName,uid);
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
    }
